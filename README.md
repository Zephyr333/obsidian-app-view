...
- **调整范围入口（零常驻污染，极简交互）**：
  ...
  - **右键编辑区（智能单项匹配）**：
    - 选中文本右键：仅显示“加入速查版”（若选区已在范围内则显示“移除速查版”）；
    - 未选文本右键：无论光标在空白处或正文，均显示“调整速查版范围”；若光标处于已标记段落内，同时提供“移除速查版”。
  - **调整过程**：底部浮现轻量悬浮工具条 `[ 速查版调整中 · 已选 N 段 | 加入选区 | 取消范围 | 完成调整 (Esc) ]`，完成或按 `Esc` 键退出后悬浮条收起。
- **纯净阅读体验**：速查版 1:1 对标 Obsidian 原生阅读模式，渲染原生内联文档标题，无任何多余文字按钮或元信息框。
